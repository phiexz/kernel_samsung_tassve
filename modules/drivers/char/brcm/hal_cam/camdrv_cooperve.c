...
#ifdef CONFIG_BCM_CAM_S5K4ECGX 
	printk("[CAM] Set Flash Mode : 0x%04x -> 0x%04x \n", stv0986_sys_flash_mode, effect);
	stv0986_sys_flash_mode = effect;
#endif
...
#ifdef CONFIG_BCM_CAM_S5K4ECGX
	if(effect == CamFocusControlAuto)				Drv_FocusMode = EXT_CFG_AF_SET_NORMAL;
	else if(effect == CamFocusControlMacro)		Drv_FocusMode = EXT_CFG_AF_SET_MACRO;
...
HAL_CAM_Result_en_t CAMDRV_SetFlash(int lux_val)
{
	...
	if(lux_val < 1) printk(KERN_ERR"     Drv_AFCanceled: %d \n", Drv_AFCanceled);

	/* setting a sensor #2*/
	if(lux_val == PRE_FLASH_OFF)
		pstSensor->m_pstAPIs->WriteDirectSensorData( pstSensor, CAMACQ_SENSORDATA_PRE_FLASH_OFF);
	else if(lux_val == FLASH_OFF && Drv_AFCanceled == TRUE)
		pstSensor->m_pstAPIs->WriteDirectSensorData( pstSensor, CAMACQ_SENSORDATA_PRE_FLASH_OFF);
	else if(lux_val == FLASH_OFF && Drv_AFCanceled == FALSE)
	...
	switch(value)
	{
      ...
      printk("%s EXT_CFG_AF_CHECK_AE_STATUS : readValue1[0] = 0x%x, readValue1[1] = 0x%x\n",__func__, readValue1[0], readValue1[1]);
		...
		case EXT_CFG_AF_OFF :
			...
			{
            CamacqExtDirectlyWriteI2cLists(pstSensor->m_pI2cClient, reg_main_set_af_normal_mode_1,0 );
				if(s5k4ecgx_status.scene == EXT_CFG_SCENE_NIGHTSHOT)mdelay(250);
				else mdelay(100);
            CamacqExtDirectlyWriteI2cLists(pstSensor->m_pI2cClient, reg_main_set_af_normal_mode_2,0 );
				if(s5k4ecgx_status.scene == EXT_CFG_SCENE_NIGHTSHOT)mdelay(250);
				else mdelay(100);
            CamacqExtDirectlyWriteI2cLists(pstSensor->m_pI2cClient, reg_main_set_af_normal_mode_3,0 );
			}
			else
			{
            CamacqExtDirectlyWriteI2cLists(pstSensor->m_pI2cClient, reg_main_set_af_macro_mode_1,0 );
				if(s5k4ecgx_status.scene == EXT_CFG_SCENE_NIGHTSHOT)mdelay(250);
				else mdelay(100);
            CamacqExtDirectlyWriteI2cLists(pstSensor->m_pI2cClient, reg_main_set_af_macro_mode_2,0 );
				if(s5k4ecgx_status.scene == EXT_CFG_SCENE_NIGHTSHOT)mdelay(250);
				else mdelay(100);
            CamacqExtDirectlyWriteI2cLists(pstSensor->m_pI2cClient, reg_main_set_af_macro_mode_3,0 );
			}
#endif         
		break;
...
HAL_CAM_Result_en_t CAMDRV_TurnOnAF()
{
   ...
   U32 sensor= 0;//Rear Camera
	...
	if( pstSensor == NULL )
	{
		...
		return HAL_CAM_ERROR_OTHERS;
                ...
                do
                {
                    ...
                    msleep(250);// skip 1 frames for AF
                    i++;
                }
                while (i<7);
	...
	pstSensor->m_pstAPIs->WriteDirectSensorData( pstSensor, CAMACQ_SENSORDATA_AWB_LOCK);
                ...
                msleep(2*250);// skip 1 frames for AF
                
                CAMDRV_SetAF(EXT_CFG_AF_CHECK_STATUS);

                for (i=0; i<100; i++) {
                    if ( ( mAFStatus == EXT_CFG_AF_SUCCESS) || (mAFStatus==EXT_CFG_AF_CANCELED)) {
...
//                        status = 0;
                    ...
                    default:
#if 0
                        if ( mAFCancel) status = 2; // Cancel == 2
                        else status = 0;//TIMEOUT or fail..etc // 0 == fail AF on App
#endif                        
                        break;
                }
